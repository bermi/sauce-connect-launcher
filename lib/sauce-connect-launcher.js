...
function run(options, callback) {
  ...
  var child,
    watcher,
    args = ["-jar", jarfile, options.username || process.env.SAUCE_CONNECT_USERNAME, options.accessKey || process.env.SAUCE_CONNECT_ACCESSKEY],
    dataActions = {
  ...
  watcher = fs.watch(__dirname, {persistent: false});
  watcher.on("error", callback);
  watcher.on("change", function (evt, filename) {
    if (filename.indexOf("readyfile") !== -1) {
      ready();
    }
  });
